...
class VF_TwoStep:
    ...
    def __init__(self, clf, reg, k=0.1, max_iter=10, min_confidence=0, convergence_threshold=0, log_level='ERROR'):
        """
        初始化 VF_TwoStep 类。
    ...
    def fit(self, XA_L, XB_L, y_L, XA_U, XB_U, task_type):
        """
        训练 TwoStep 模型。
        ...
        """
        # 日志打印有标签和无标签数据的样本数量
        self.logger.info("有标签数据 (L): %d 样本", XA_L.shape[0])
        self.logger.info("无标签数据 (U): %d 样本", XA_U.shape[0])
        ...
        if task_type == "classification":
            ...
            # 如果无法确定任务类型，参数类型错误
            self.logger.warning("无法确定任务类型，参数类型错误。")
            raise TypeError(f"task_type = {task_type}, 参数类型错误")

        self.logger.info("VF_TwoStep 模型训练完成。")
        return self
    ...
    def __fit__clf(self, XA_L, XB_L, y_L, XA_U, XB_U):
        ...
        self.logger.info("[INIT] 初始化 self.pred, 未标注数据数量=%d", len(XA_U))
        self.logger.info("[INIT] 最大迭代次数 max_iter=%d, 每轮选取比例 k=%.2f", self.max_iter, self.k)
        ...
        for epoch in range(self.max_iter):
            ...
            # 1. 训练分类器
            self.logger.info("[STEP 1] 开始训练分类器 (基于 %d 个 Labeled 样本)...", len(XA_L))
            clf_start_time = time.time()
            self.clf.fit(XA_L, XB_L, y_L)
            clf_time = time.time() - clf_start_time
            self.logger.info("[STEP 1] 分类器训练完成，耗时 %.2f 秒。", clf_time)

            # 打印一下分类器识别到的类别信息(若有)
            if hasattr(self.clf, 'classes_'):
                self.logger.debug("[DEBUG] 分类器识别的类别列表: %s", self.clf.classes_)
            else:
                self.logger.debug("[DEBUG] 分类器无 'classes_' 属性，无法输出类别列表。")

            # 2. 对 Unlabeled 数据打分
            self.logger.info("[STEP 2] 对 Unlabeled 数据进行预测概率计算...")
            ...
            self.logger.info("[STEP 2] 预测概率计算完成，耗时 %.2f 秒。", scores_time)
            ...
            self.logger.info(
                "[STEP 5] 从 Unlabeled 集中删除这些样本,剩余 Unlabeled len(XA_U)=%d,len(XB_U)=%d, len(unlabeled_indices)=%d",
        ...
        if len(XA_U) != 0:
            self.logger.info("[FINAL] 对剩余 %d 个未标记样本进行最终预测... unlabeled_indices 大小为 %d", len(XA_U), len(unlabeled_indices))
    ...
    def __fit__reg(self, XA_L, XB_L, y_L, XA_U, XB_U):
        ...
        for epoch in range(self.max_iter):
            ...
            # 1. 训练回归器
            self.logger.info("[STEP 1] 开始训练回归器 (基于 %d 个 Labeled 样本)...", len(XA_L))
            reg_start_time = time.time()
            self.reg.fit(XA_L, XB_L, y_L)
            clf_time = time.time() - reg_start_time
            self.logger.info("[STEP 1] 回归器训练完成，耗时 %.2f 秒。", clf_time)

            # 2. 对 Unlabeled 数据打分
            self.logger.info("[STEP 2] 对 Unlabeled 数据进行置信度打分...")
            scores_start_time = time.time()
